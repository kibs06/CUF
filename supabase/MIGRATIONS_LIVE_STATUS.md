...
**✅ 2026-09-18 — `20260918193313_add_product_audience.sql` is APPLIED (verified live by object, not by trust).** Phase 0 of `docs/AI/PRODUCT_AUDIENCE_PLAN.md`: `products.audience` → **`text`, nullable**, and the constraint **`products_audience_check`** present with `CHECK ((audience = ANY (ARRAY['men','women','kids','unisex'])))`; `products` is at **22 columns** (21 before). The CHECK was written **twice on purpose** (inline + explicit `DROP CONSTRAINT IF EXISTS` / `ADD CONSTRAINT`) so it **converges** on a re-apply — the rule in *A table is not re-created by re-running* below, where an inline CHECK beside an `ADD COLUMN IF NOT EXISTS` is a no-op the second time. **Additive only:** no backfill (an audience is stated by a seller, never inferred from the size band), no index, no policy change, RLS unaffected. **Nothing reads or writes it yet** — the seller chip group is P1 and the home rails are P2, both behind `AppConstants.productAudienceEnabled = false` — so no shipped build can be affected by this landing. Applied via SQL Editor.

**⚠️ 2026-09-17 — `20260917120000_add_pickup_reservation_color.sql` is NOT applied yet.**
It is additive and safe to run, but read this order note first: the file **drops
`request_pickup_reservation(UUID, TEXT, INTEGER)`** and recreates it with a
fourth `p_color TEXT DEFAULT NULL` argument, because Postgres treats a 3-argument
call as AMBIGUOUS against both signatures ("function is not unique", 42725)
rather than preferring one. Nothing else changes: the table only gains a
nullable `color` column, the hold stays keyed on `(product_id, size)`, and
PostgREST resolves by argument NAME, so every already-shipped client keeps
working with the colour simply absent. Until it is applied the app still creates
holds — `PickupReservationService.request` retries without `p_color` when the
server answers "function not found" — and the colour is simply not recorded.
Companion code: the colour picker in both reservation sheets, and the colour
line on the seller's pickup tiles and the customer's My Pickup Reservations.
...
**✅ 2026-09-17 — amended-file re-apply: all 8 accounted for (7 verified live, the 8th deferred with pg_cron).** Asserted against `pg_proc.prosrc`: `notify_on_order_status_change` (null guard) ✅, `notify_on_order_insert` (pending guard) ✅, `cancel_pickup_reservation` (`v_recipient IS NOT NULL`) ✅, the T5 confirm (`Guarded on the RECIPIENT`) ✅, both bulk files (`s.owner_id IS NOT NULL`, ×4) ✅, and their policies ✅. **The eighth — `20260809000000_revive_paymongo_online_gcash.sql` — needs nothing applied, and searching `cancel_my_pending_payment_intent` for it will NOT find it: that file's `WHERE o.customer_id IS NOT NULL` guard lives inside the `expire-online-gcash-payments` pg_cron job body (the `$cron$ … $cron$` block, lines ~395–508), not in any function.** With `pg_cron` absent the whole `DO $$ … $$` registration degrades to a NOTICE, so the guard is never stored and the vulnerable statement is never scheduled. It arrives with pg_cron — the same hand-run step as `20260817150000` below.
...
widened CHECK.
**✅ 2026-09-17: 7 of the 8 are verified live, and the 8th (`20260809000000`, whose guard is a pg_cron job body) needs nothing while pg_cron is absent — see the 2026-09-17 block above.**
...
| `20260918193313_add_product_audience.sql` | ✅ **Applied** (verified live Sep 18, 2026, by object: `audience` = `text`, nullable; `products_audience_check` present with the four-value CHECK; `products` = 22 columns) | **Product audience P0** — `products.audience` (nullable, `'men' \| 'women' \| 'kids' \| 'unisex'`), the value the Men's/Women's/Kids' home rails (P2) and the audience-aware US/UK chart (P3) will read. **No backfill on purpose** — `NULL` means not set and shows in no rail; inferring it from the size band is the shortcut the plan rejects (EU is unisex, so Men's and Women's share one band and the kids' band overlaps it). The CHECK is written **twice on purpose** (inline + explicit DROP/ADD) so a re-apply converges it. No index, no policy change. Companion Dart: `lib/utils/product_audience.dart` + `AppConstants.productAudienceEnabled` (still `false`). See `docs/AI/PRODUCT_AUDIENCE_PLAN.md` §4 P0. Apply via SQL Editor. |
