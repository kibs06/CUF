...
**Last verified:** September 16, 2026 — every row re-checked against the live
...
(92 migrations, from scratch) then `supabase test db` → **`Result: PASS`**,
8 files / 359 tests (`pickup_reservations` 129, `bulk_reservation_deposits` 51,
`trusted_device_enforcement` 44, `vouchers` 37, `admin_account_security` 31,
`trusted_devices` 30, manual-GCash dedupe 30, RLS recursion canary 7).
Flutter side: `flutter analyze` clean,
`flutter test` 846 PASS. The *rows below* track
...
| `20260817150000_add_store_auto_schedule_cron.sql` | ⏳ **Still not applied — blocked** (verified Sep 16, 2026) | Schedules the `apply-store-schedules` pg_cron job (every 5 min) so `stores.is_open` auto-flips per `open_time`/`close_time`. Fixes stores stuck showing "Open Now" after their posted close time. Apply via Dashboard → SQL Editor |
