...
- **What may move that counter (fixed Sep 17, 2026 after a playtest).** Measured on the Pixel 4 emulator: with the device in airplane mode — the login screen's own banner already read *"No internet connection"* — **five taps on "Log In" produced the full 30-minute lockout**, overlay copy included ("Multiple failed login attempts were detected from a device"), with no password ever leaving the device; the local counter, the lockout e-mail, the push and the admin `failed_logins` row all fired for a network outage, and the **correct** password was then refused locally for 30 minutes. The catch block counted *every* exception, so transport failures were indistinguishable from guessing. It now counts only a definite credential rejection: `isCredentialRejection()` in `lib/utils/auth_error_messages.dart` (true for `invalid_credentials` / `user_not_found`, false for `AuthRetryableFetchException`, socket, timeout, 429 and 5xx). `friendlyAuthErrorMessage` likewise renders transport failures as a connection message rather than blaming the password. Residual, by design: five genuine wrong-password attempts still lock the account for 30 minutes.
