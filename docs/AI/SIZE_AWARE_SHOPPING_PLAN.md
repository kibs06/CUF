...
| **P2** — discovery | ⚠️ **The rail only, where the plan proposed a chip + badge.** `lib/widgets/in_your_size_section.dart` — “In your size”, a horizontal rail of products that stock the customer's size right now, behind the one-const switch `AppConstants.sizeAwareShoppingEnabled` (the `kBestSellersRailEnabled` shape). No card badge, no filter chip, no `kSizeAwareShoppingEnabled` under that name. |
