...
| Surface | File |
|---|---|
| **Reserve for Pickup** action on the product detail screen (product summary row, colour picker when the product has variants, then quantity stepper capped at 2 *and* at the size's live stock — the stock wins when it is lower). The hold records the chosen colour, which is what tells the counter WHICH pair to set aside; it is informational, since the hold and its stock check are keyed on `(product_id, size)` and `inventory` is colour-blind | `lib/screens/customer/widgets/pickup_reservation_sheet.dart` |
