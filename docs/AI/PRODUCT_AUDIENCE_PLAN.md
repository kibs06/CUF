...
**Schema change:** one nullable column on `public.products` (+ a `CHECK`). **The migration must be applied before the client build that writes it** — a missing column fails the write and the client reports it as a generic error (see §5 R5, the exact failure `20260917120000_add_foot_size_category.sql` produced).
...
| R5 | **Migration ordering** — a build that writes a column the project does not have fails the write, and the customer/seller sees a generic "Something went wrong" | Apply the migration first; call it out in the CHANGELOG and the release checklist; this is the exact failure `20260917120000_add_foot_size_category.sql` produced |
