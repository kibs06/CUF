...
**The target no longer has to be dead air (Sep 21, 2026).** The ≥40px has to come from *inside* the tape's own box: Flutter delivers a tap only within every ancestor's bounds, so slack cannot be borrowed from a parent and no overlay can reach outside its box. On a product card the padding-only version therefore opened ~18px between the rating row and the price and another ~8px between the two prices — invisible in the hit area, plainly visible on the card. `SalePriceTape.targetBelow` is the fix: a line that *belongs* with the price shares the target without the tape covering it, so `SoleProductCard` passes the strikethrough original price as `targetBelow` with `hitPadding: EdgeInsets.symmetric(vertical: 4)` and the ≥40px is the two price lines together (~44px). The visual's insets are measured against the price's own box (not the target's), so the tape still hugs the number alone. The other call sites are unchanged and still use padding: the detail screen `fromLTRB(10, 22, 10, 0)` and the Recently Viewed strip `fromLTRB(10, 20, 10, 9)`.

**The padding is vertical only, and the category rides the number's line (Sep 21, 2026).** Two alignment defects in the same block, both from the block being assembled out of parts that do not know about each other:

- **The horizontal half of the padding moved the NUMBER, not the target's boundary.** A padded box is padded *around* its text, so `hitPadding.left` shifted the price 10px right of the original under it, the name above it, and the same price on a card with no sale — the *"the discounted price is slightly off"* report. The tape's own overhang is now a constant measured from the **text** (`_overhangTop` / `_overhangBottom` / `_overhangSide`), not derived from `hitPadding`, so the card drops the horizontal half: the digits move left to the card's own left edge and the strip stays exactly where it was (10px past the digits on each side, 8px above and 4px below them). A caller that still pads horizontally is unaffected — its visual insets come out the same as before.
- **The category label was centered against the tape's two-line box, not the number.** `SoleProductCard`'s row centers its children against the tape's box, which is right when both children are one line (a card with no sale) and half a line low when the first child is the tape's two lines — the label landed on the seam between the two prices, the *"the sandals didn't align"* report. The label now belongs to the **tape** (`SalePriceTape.sameLine`), the only widget that knows where the number's line is: it shares the number's row, so it is centered on the number — the same place it sits on a card with no sale — and where the two cannot share a row it takes a line of its own above the original, never on the seam. Both outside fixes were tried and both are worse, which is why the tape owns it: giving the label a box shaped like the tape's (a line plus an invisible twin of the original's) is exact while it rides, but costs the block a whole line the moment it wraps — measured clipping the sale grid's cell at 2.0× text scale — and nudging it up with a transform drops it onto the price above when it wraps. The label is **not** part of the reveal: the tap target is the number plus the line under it, and the label carries its own hit area, so a tap on it is the caller's (a card opens the product).
...
7. **Never change the price block's footprint between states.** The sale-price `Text` sits in a padded box (`hitPadding`) and the SAME box is returned in the covered and revealed states; the tape visual is a `Positioned`/`IgnorePointer` overlay (`clipBehavior: Clip.none`) hugging the text alone — its insets are measured against the price's box, never the target's, so a `targetBelow` line is never covered. Covered and revealed must be pixel-identical in size, and the original/strikethrough line is never covered. The **≥40px tall tap area is the rule** (not the padding that used to provide it): reach it with padding where the slack will not show, or with `targetBelow` where it would — that is what `SoleProductCard` does (`hitPadding: EdgeInsets.symmetric(vertical: 4)` plus the original price as the second line of the target), because the padding-only version put ~18px of dead air between the rating row and the price. **Pad vertically, never horizontally, where the price must line up with the rest of the card**: the padding moves the *text*, so a horizontal inset shifts the number off the left edge its name, its original price and every other card share. The tape's visual overhang does not follow the padding (`_overhangTop` / `_overhangBottom` / `_overhangSide` are measured from the text), so dropping it is free. And a one-line label centered beside the tape lands on the seam between the two prices — hand it to `SalePriceTape.sameLine` instead of centering it against the tape from outside, because the tape's box is two lines tall and nothing outside it knows where the number's line is. (Shaping the label's own box like the tape's is exact while they share a line but costs the block a line when the label wraps, and a transform that nudges it up drops it onto the price above — both were measured against the real card before `sameLine` was added.) The detail screen still puts its slack above (`fromLTRB(10, 22, 10, 0)`) to preserve its bottom-aligned price row; the strip uses `fromLTRB(10, 20, 10, 9)` for its 11px price. A `targetBelow` also means the target's semantics: the reveal label is its own node (`container: true`, `explicitChildNodes` when there is a line below) so the original price keeps its own announcement while the covered number stays hidden.
