...
class _StoreProfileScreenState extends State<StoreProfileScreen> {
  ...
  List<Map<String, dynamic>> get _collections {
    final cats = <String>{};
    for (final p in _storeProducts) {
      cats.add(p['category'] as String);
    }
    final icons = {
      'Classic': Icons.auto_awesome,
      'Loafers': Icons.weekend_outlined,
      'Boots': Icons.directions_walk,
      'Derby': Icons.workspace_premium,
      'Sandals': Icons.wb_sunny_outlined,
    };
    final colors = {
      'Classic': const Color(0xFF8B5A2B),
      'Loafers': const Color(0xFF4A3728),
      'Boots': const Color(0xFF3B2314),
      'Derby': const Color(0xFF6B8F47),
      'Sandals': const Color(0xFF4ECDC4),
    };
    return cats.map((cat) {
      return {
        'name': cat,
        'icon': icons[cat] ?? Icons.category,
        'color': colors[cat] ?? AppConstants.primary,
      };
    }).toList();
  }
        ...
        child: CustomScrollView(
                        ...
                        Navigator.of(context).push(
                          MaterialPageRoute(
                            builder: (_) => CollectionScreen(
                              ...
                              categoryFilter: col['name'] as String,
                              storeId: widget.storeId,
                            ),
                          ),
                        );
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          gradient: LinearGradient(
                            ...
                            colors: [
                              (col['color'] as Color).withAlpha(220),
                              (col['color'] as Color).withAlpha(140),
                            ],
                          ),
                        ),
                        child: Column(
          ...
          SliverToBoxAdapter(
            child: SingleChildScrollView(
              ...
              padding: const EdgeInsets.symmetric(horizontal: 16),
              child: Row(
                children: ['Newest', 'Price: Low–High', 'Price: High–Low'].map((
                  mode,
                ) {
